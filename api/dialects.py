...
class S3SQLiteDialect(SQLiteDialect_pysqlite):
    ...
    def load_remote_db(self, dbname=None):
        ...
        print("S3SQLiteDialect.load_remote_db(self,dbname='{dbname}')".format(dbname=dbname))
        ...
        if '/tmp/' not in self._local_dbname:
            try:
                ...
                print("Reading from s3 bucket: " + bucketname)
                print("DB Name: " + self._local_dbname)
    ...
    def close(self, *args, **kwargs):
        ...
        """
        print("S3SQLiteDialect.load_remote_db(self, *args, **kwargs)")
        bucketname = os.environ.get('S3SQLite_bucket')
        ...
        try:
            with open(self._local_dbname, 'rb') as f:
                ...
                if self.db_hash == m.hexdigest():
                    print("Database unchanged, not saving to remote DB!")
                    return

                print("Database changed, Saving to remote DB!")
                bytesIO = BytesIO()
                bytesIO.write(fb)
                bytesIO.seek(0)
                ...
                result = s3_object.put(ACL='private', Body=bytesIO)
                print(result)
                print("Saved to remote DB!")
        except Exception as e:
            logging.debug(e)
